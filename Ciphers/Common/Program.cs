...
using RSA = Ciphers.Ciphers.Implementations.RSA;
...
public class Program
{
    ...
    {
        string phrase = "Security";
        var byteMessage = ASCIIEncoding.ASCII.GetBytes(phrase);

        string key_phrase = "Publicity";

        var key = ASCIIEncoding.ASCII.GetBytes(key_phrase);
        ...
        // var text = Convert.ToBase64String(rc4Encrypted);
        // var rc4Decrypted = new Rc4(key).Decrypt(rc4Encrypted,rc4Encrypted.Length);
        // var decryptedText = ASCIIEncoding.ASCII.GetString(rc4Decrypted);
        // Console.WriteLine($"The encrypted message using rc4 cipher is : '{text}'");
        ...
        RsaKeyPair Rsakey = RSA.GenerateKeyPair(2048);
        byte[] enc = RSA.Encrypt(text, Rsakey.Public);

        Console.WriteLine("[Encrypted]");
        Console.WriteLine(Convert.ToBase64String(enc) + "\n");

        byte[] dec = RSA.Decrypt(enc, Rsakey.Private);
